...
OUTPUT_PATH = RESULTS_DIR / "block_scoring_summary.json"
BLOCK_SIZE = 128
TOP_K = 4
...
def main() -> None:
    ...
    prompt = "RelayKV scoring test. " * 64
    ...
    # 最初は layer 0 の最後の hot token を query 代わりに使う
    # shape: [batch, heads, seq_len, head_dim]
    query = layers[0].keys[:, :, -1, :]   # [1, heads, head_dim]
    ...
    summary = {
        ...
        "num_layer0_blocks": len(layer0_metadata),
        "top_k": TOP_K,
        "query_shape": list(query.shape),
        "top_blocks": [s.summary() for s in top_scores],
    }
