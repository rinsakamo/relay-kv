...
def torch_dtype_from_string(dtype_str: str | None) -> torch.dtype | None:
...
def prepare_runtime_bridge_hidden_state(
    *,
    bridge_hidden_state: torch.Tensor,
    ...
    if len(target_shape) != 3:
        return None, "unexpected_target_rank"

    if list(bridge_hidden_state.shape) != list(target_shape):
        return None, "shape_mismatch"

    runtime_bridge = bridge_hidden_state

    if target_device is not None:
        runtime_bridge = runtime_bridge.to(device=torch.device(target_device))

    target_torch_dtype = torch_dtype_from_string(target_dtype)
    if target_dtype is not None and target_torch_dtype is None:
        return None, "unsupported_target_dtype"
    if target_torch_dtype is not None:
        runtime_bridge = runtime_bridge.to(dtype=target_torch_dtype)

    return runtime_bridge, None
...
def make_layer14_oproj_pre_hook(state: Layer14InjectionState):
    def _hook(module, inputs):
        ...
        state.oproj_pre_hook_replaced = False

        if (
            state.enabled
            and state.injection_tensor is not None
            and state.injection_target == "layer14_oproj_input"
        ):
    ...
    for step_idx in range(max_new_tokens):
        ...
        if replacement_hook_triggered:
            ...
            if tensor_writeback_ready:
                ...
                runtime_bridge_hidden_state, runtime_bridge_reason = prepare_runtime_bridge_hidden_state(
                    bridge_hidden_state=bridge_hidden_state,
                    target_shape=injection_state.oproj_pre_last_seen_shape,
                    target_device=injection_state.oproj_pre_last_seen_device,
                    target_dtype=injection_state.oproj_pre_last_seen_dtype,
                )

                layer14_eval["replacement_bridge_runtime_shape"] = (
                    list(runtime_bridge_hidden_state.shape)
                    if runtime_bridge_hidden_state is not None
                    else None
                )
                layer14_eval["replacement_bridge_runtime_device"] = (
                    str(runtime_bridge_hidden_state.device)
                    if runtime_bridge_hidden_state is not None
                    else None
                )
                layer14_eval["replacement_bridge_runtime_dtype"] = (
                    str(runtime_bridge_hidden_state.dtype)
                    if runtime_bridge_hidden_state is not None
                    else None
                )
                layer14_eval["replacement_bridge_runtime_reason"] = runtime_bridge_reason
                ...
                layer14_eval["replacement_bridge_compare"] = {
                    ...
                    "bridge_hidden_state_device": str(bridge_hidden_state.device),
                    "bridge_hidden_state_dtype": str(bridge_hidden_state.dtype),
                    "bridge_runtime_shape": (
                        list(runtime_bridge_hidden_state.shape)
                        if runtime_bridge_hidden_state is not None
                        else None
                    ),
                    "bridge_runtime_device": (
                        str(runtime_bridge_hidden_state.device)
                        if runtime_bridge_hidden_state is not None
                        else None
                    ),
                    "bridge_runtime_dtype": (
                        str(runtime_bridge_hidden_state.dtype)
                        if runtime_bridge_hidden_state is not None
                        else None
                    ),
                    "bridge_runtime_reason": runtime_bridge_reason,
                    "target_shape": injection_state.oproj_pre_last_seen_shape,
                    "target_device": injection_state.oproj_pre_last_seen_device,
                    "target_dtype": injection_state.oproj_pre_last_seen_dtype,
                ...
                layer14_eval["replacement_injection_selected"] = runtime_bridge_hidden_state is not None
                layer14_eval["replacement_injection_target"] = "layer14_oproj_input"
                ...
                if runtime_bridge_hidden_state is not None:
                    ...
                    injection_ready, injection_reason = False, runtime_bridge_reason

                layer14_eval["replacement_injection_ready"] = injection_ready
                layer14_eval["replacement_injection_ready_reason"] = injection_reason
                layer14_eval["replacement_injection_tensor_shape"] = (
                    list(runtime_bridge_hidden_state.shape)
                    if runtime_bridge_hidden_state is not None
                    else None
                )
                layer14_eval["replacement_injection_tensor_device"] = (
                    str(runtime_bridge_hidden_state.device)
                    if runtime_bridge_hidden_state is not None
                    else None
                )
                layer14_eval["replacement_injection_tensor_dtype"] = (
                    str(runtime_bridge_hidden_state.dtype)
                    if runtime_bridge_hidden_state is not None
                    else None
                )

                if injection_ready and runtime_bridge_hidden_state is not None:
                ...
                layer14_eval["replacement_bridge_runtime_shape"] = None
                layer14_eval["replacement_bridge_runtime_device"] = None
                layer14_eval["replacement_bridge_runtime_dtype"] = None
                layer14_eval["replacement_bridge_runtime_reason"] = None
        ...
        injection_state.enabled = False
        injection_state.injection_tensor = None
        injection_state.injection_target = None
...
def main() -> None:
    ...
    summary = {
        ...
        "decode_policy": {
            ...
            "injection_stage": "runtime_bridge_ready",
        },
        "max_new_tokens": args.max_new_tokens,
