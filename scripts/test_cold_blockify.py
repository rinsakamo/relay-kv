...
OUTPUT_PATH = RESULTS_DIR / "cold_blocks_summary.json"
BLOCK_SIZE = 128
...
def main() -> None:
    ...
    prompt = "RelayKV cold block test. " * 64
        ...
        "first_five_blocks": [
            {
                ...
                "end": b.end,
                "length": b.length,
                "k_shape": list(b.k.shape) if b.k is not None else None,
                "v_shape": list(b.v.shape) if b.v is not None else None,
                "k_device": str(b.k.device) if b.k is not None else None,
                "v_device": str(b.v.device) if b.v is not None else None,
            }
            for b in blocks[:5]
        ],
    }
